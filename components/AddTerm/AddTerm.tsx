        ...
        <div className="flex flex-col mx-auto xs:w-10/12 md:w-1/2 lg:w-1/3 gap-y-3.5">
            <form className="pt-20" onSubmit={handleSubmit(onSubmit)}>
                ...
                <InputWrapper labelId={'language'} labelText={'Language'}>
                    <select
                        {...register('language')}
                        className="py-2 px-3 bg-dark-medium border-main-green border-2 rounded-lg border-solid w-full text-light-grey font-normal"
                        id="languages"
                        onChange={dropdownChangeHandler}
                    >
                        <option selected value={Language.HEBREW}>
                            עברית
                        </option>
